...
| `excludeStrategy` | `?string` | Optional | If an `exclude_products_id` was given, controls which subset of matched<br>products is excluded from any discounts.<br><br>Default value: `LEAST_EXPENSIVE` | getExcludeStrategy(): ?string | setExcludeStrategy(?string excludeStrategy): void |
...
```php
use SquareConnectAPILib\Models\Builders\CatalogPricingRuleBuilder;

$catalogPricingRule = CatalogPricingRuleBuilder::init()
    ->applyProductsId('apply_products_id4')
    ->customerGroupIdsAny(
        [
    ...
    ->excludeProductsId('exclude_products_id6')
    ->excludeStrategy('exclude_strategy8')
    ->build();
```
