...
| `inventoryAlertType` | `?string` | Optional | Indicates whether the item variation displays an alert when its inventory quantity is less than or equal<br>to its `inventory_alert_threshold`. | getInventoryAlertType(): ?string | setInventoryAlertType(?string inventoryAlertType): void |
...
| `name` | `?string` | Optional | The item variation's name. This is a searchable attribute for use in applicable query filters, and its value length is of Unicode code points.<br><br>**Constraints**: *Maximum Length*: `255` | getName(): ?string | setName(?string name): void |
...
| `pricingType` | `?string` | Optional | Indicates whether the item variation's price is fixed or determined at the time<br>of sale. | getPricingType(): ?string | setPricingType(?string pricingType): void |
...
| `userData` | `?string` | Optional | Arbitrary user metadata to associate with the item variation. This attribute value length is of Unicode code points.<br><br>**Constraints**: *Maximum Length*: `255` | getUserData(): ?string | setUserData(?string userData): void |

## Example

```php
use SquareConnectAPILib\Models\Builders\CatalogItemVariationBuilder;
use SquareConnectAPILib\Models\Builders\CatalogItemOptionValueForItemVariationBuilder;

$catalogItemVariation = CatalogItemVariationBuilder::init()
    ->availableForBooking(false)
    ->inventoryAlertThreshold(90)
    ->inventoryAlertType('inventory_alert_type8')
    ->itemId('item_id4')
    ->itemOptionValues(
        [
            CatalogItemOptionValueForItemVariationBuilder::init()
                ->itemOptionId('item_option_id0')
                ->itemOptionValueId('item_option_value_id2')
                ->build()
        ]
    )
    ->build();
