# Labor

```php
$laborController = $client->getLaborController();
```

## Class Name

`LaborController`

## Methods

* [List Break Types](../../doc/controllers/labor.md#list-break-types)
* [Create Break Type](../../doc/controllers/labor.md#create-break-type)
* [Delete Break Type](../../doc/controllers/labor.md#delete-break-type)
* [Get Break Type](../../doc/controllers/labor.md#get-break-type)
* [Update Break Type](../../doc/controllers/labor.md#update-break-type)
* [List Employee Wages](../../doc/controllers/labor.md#list-employee-wages)
* [Get Employee Wage](../../doc/controllers/labor.md#get-employee-wage)
* [Create Shift](../../doc/controllers/labor.md#create-shift)
* [Search Shifts](../../doc/controllers/labor.md#search-shifts)
* [Delete Shift](../../doc/controllers/labor.md#delete-shift)
* [Get Shift](../../doc/controllers/labor.md#get-shift)
* [Update Shift](../../doc/controllers/labor.md#update-shift)
* [List Team Member Wages](../../doc/controllers/labor.md#list-team-member-wages)
* [Get Team Member Wage](../../doc/controllers/labor.md#get-team-member-wage)
* [List Workweek Configs](../../doc/controllers/labor.md#list-workweek-configs)
* [Update Workweek Config](../../doc/controllers/labor.md#update-workweek-config)


# List Break Types

Returns a paginated list of `BreakType` instances for a business.

```php
function listBreakTypes(
    ?string $locationId = null,
    ?int $limit = null,
    ?string $cursor = null
): ListBreakTypesResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `locationId` | `?string` | Query, Optional | Filter the returned `BreakType` results to only those that are associated with the<br>specified location. |
| `limit` | `?int` | Query, Optional | The maximum number of `BreakType` results to return per page. The number can range between 1<br>and 200. The default is 200. |
| `cursor` | `?string` | Query, Optional | A pointer to the next page of `BreakType` results to fetch. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_READ`

## Response Type

**200**: Success

[`ListBreakTypesResponse`](../../doc/models/list-break-types-response.md)

## Example Usage

```php
$laborController = $client->getLaborController();

try {
    $result = $laborController->listBreakTypes();
    echo 'ListBreakTypesResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Create Break Type

Creates a new `BreakType`.

A `BreakType` is a template for creating `Break` objects.
You must provide the following values in your request to this
endpoint:

- `location_id`
- `break_name`
- `expected_duration`
- `is_paid`

You can only have three `BreakType` instances per location. If you attempt to add a fourth
`BreakType` for a location, an `INVALID_REQUEST_ERROR` "Exceeded limit of 3 breaks per location."
is returned.

```php
function createBreakType(CreateBreakTypeRequest $body): CreateBreakTypeResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `body` | [`CreateBreakTypeRequest`](../../doc/models/create-break-type-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_WRITE`

## Response Type

**200**: Success

[`CreateBreakTypeResponse`](../../doc/models/create-break-type-response.md)

## Example Usage

```php
$body = CreateBreakTypeRequestBuilder::init(
    BreakTypeBuilder::init(
        'break_name0',
        'expected_duration6',
        false,
        'location_id2'
    )->build()
)->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->createBreakType($body);
    echo 'CreateBreakTypeResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Delete Break Type

Deletes an existing `BreakType`.

A `BreakType` can be deleted even if it is referenced from a `Shift`.

```php
function deleteBreakType(string $id): DeleteBreakTypeResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `BreakType` being deleted. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_WRITE`

## Response Type

**200**: Success

[`DeleteBreakTypeResponse`](../../doc/models/delete-break-type-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->deleteBreakType($id);
    echo 'DeleteBreakTypeResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Get Break Type

Returns a single `BreakType` specified by `id`.

```php
function getBreakType(string $id): GetBreakTypeResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `BreakType` being retrieved. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_READ`

## Response Type

**200**: Success

[`GetBreakTypeResponse`](../../doc/models/get-break-type-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->getBreakType($id);
    echo 'GetBreakTypeResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Update Break Type

Updates an existing `BreakType`.

```php
function updateBreakType(string $id, UpdateBreakTypeRequest $body): UpdateBreakTypeResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `BreakType` being updated. |
| `body` | [`UpdateBreakTypeRequest`](../../doc/models/update-break-type-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_READ`, `TIMECARDS_SETTINGS_WRITE`

## Response Type

**200**: Success

[`UpdateBreakTypeResponse`](../../doc/models/update-break-type-response.md)

## Example Usage

```php
$id = 'id0';

$body = UpdateBreakTypeRequestBuilder::init(
    BreakTypeBuilder::init(
        'break_name0',
        'expected_duration6',
        false,
        'location_id2'
    )->build()
)->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->updateBreakType(
        $id,
        $body
    );
    echo 'UpdateBreakTypeResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# List Employee Wages

Returns a paginated list of `EmployeeWage` instances for a business.

```php
function listEmployeeWages(
    ?string $employeeId = null,
    ?int $limit = null,
    ?string $cursor = null
): ListEmployeeWagesResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `employeeId` | `?string` | Query, Optional | Filter the returned wages to only those that are associated with the specified employee. |
| `limit` | `?int` | Query, Optional | The maximum number of `EmployeeWage` results to return per page. The number can range between<br>1 and 200. The default is 200. |
| `cursor` | `?string` | Query, Optional | A pointer to the next page of `EmployeeWage` results to fetch. |

## Requires scope

### oauth2

`EMPLOYEES_READ`

## Response Type

**200**: Success

[`ListEmployeeWagesResponse`](../../doc/models/list-employee-wages-response.md)

## Example Usage

```php
$laborController = $client->getLaborController();

try {
    $result = $laborController->listEmployeeWages();
    echo 'ListEmployeeWagesResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Get Employee Wage

Returns a single `EmployeeWage` specified by `id`.

```php
function getEmployeeWage(string $id): GetEmployeeWageResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `EmployeeWage` being retrieved. |

## Requires scope

### oauth2

`EMPLOYEES_READ`

## Response Type

**200**: Success

[`GetEmployeeWageResponse`](../../doc/models/get-employee-wage-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->getEmployeeWage($id);
    echo 'GetEmployeeWageResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Create Shift

Creates a new `Shift`.

A `Shift` represents a complete workday for a single employee.
You must provide the following values in your request to this
endpoint:

- `location_id`
- `employee_id`
- `start_at`

An attempt to create a new `Shift` can result in a `BAD_REQUEST` error when:

- The `status` of the new `Shift` is `OPEN` and the employee has another
  shift with an `OPEN` status.
- The `start_at` date is in the future.
- The `start_at` or `end_at` date overlaps another shift for the same employee.
- The `Break` instances are set in the request and a break `start_at`
  is before the `Shift.start_at`, a break `end_at` is after
  the `Shift.end_at`, or both.

```php
function createShift(CreateShiftRequest $body): CreateShiftResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `body` | [`CreateShiftRequest`](../../doc/models/create-shift-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_WRITE`

## Response Type

**200**: Success

[`CreateShiftResponse`](../../doc/models/create-shift-response.md)

## Example Usage

```php
$body = CreateShiftRequestBuilder::init(
    ShiftBuilder::init(
        'start_at6'
    )->build()
)->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->createShift($body);
    echo 'CreateShiftResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Search Shifts

Returns a paginated list of `Shift` records for a business.
The list to be returned can be filtered by:

- Location IDs.
- Employee IDs.
- Shift status (`OPEN` and `CLOSED`).
- Shift start.
- Shift end.
- Workday details.

The list can be sorted by:

- `start_at`.
- `end_at`.
- `created_at`.
- `updated_at`.

```php
function searchShifts(SearchShiftsRequest $body): SearchShiftsResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `body` | [`SearchShiftsRequest`](../../doc/models/search-shifts-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_READ`

## Response Type

**200**: Success

[`SearchShiftsResponse`](../../doc/models/search-shifts-response.md)

## Example Usage

```php
$body = SearchShiftsRequestBuilder::init()->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->searchShifts($body);
    echo 'SearchShiftsResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Delete Shift

Deletes a `Shift`.

```php
function deleteShift(string $id): DeleteShiftResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `Shift` being deleted. |

## Requires scope

### oauth2

`TIMECARDS_WRITE`

## Response Type

**200**: Success

[`DeleteShiftResponse`](../../doc/models/delete-shift-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->deleteShift($id);
    echo 'DeleteShiftResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Get Shift

Returns a single `Shift` specified by `id`.

```php
function getShift(string $id): GetShiftResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `Shift` being retrieved. |

## Requires scope

### oauth2

`TIMECARDS_READ`

## Response Type

**200**: Success

[`GetShiftResponse`](../../doc/models/get-shift-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->getShift($id);
    echo 'GetShiftResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Update Shift

Updates an existing `Shift`.

When adding a `Break` to a `Shift`, any earlier `Break` instances in the `Shift` have
the `end_at` property set to a valid RFC-3339 datetime string.

When closing a `Shift`, all `Break` instances in the `Shift` must be complete with `end_at`
set on each `Break`.

```php
function updateShift(string $id, UpdateShiftRequest $body): UpdateShiftResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The ID of the object being updated. |
| `body` | [`UpdateShiftRequest`](../../doc/models/update-shift-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_READ`, `TIMECARDS_WRITE`

## Response Type

**200**: Success

[`UpdateShiftResponse`](../../doc/models/update-shift-response.md)

## Example Usage

```php
$id = 'id0';

$body = UpdateShiftRequestBuilder::init(
    ShiftBuilder::init(
        'start_at6'
    )->build()
)->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->updateShift(
        $id,
        $body
    );
    echo 'UpdateShiftResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# List Team Member Wages

Returns a paginated list of `TeamMemberWage` instances for a business.

```php
function listTeamMemberWages(
    ?string $teamMemberId = null,
    ?int $limit = null,
    ?string $cursor = null
): ListTeamMemberWagesResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `teamMemberId` | `?string` | Query, Optional | Filter the returned wages to only those that are associated with the<br>specified team member. |
| `limit` | `?int` | Query, Optional | The maximum number of `TeamMemberWage` results to return per page. The number can range between<br>1 and 200. The default is 200. |
| `cursor` | `?string` | Query, Optional | A pointer to the next page of `EmployeeWage` results to fetch. |

## Requires scope

### oauth2

`EMPLOYEES_READ`

## Response Type

**200**: Success

[`ListTeamMemberWagesResponse`](../../doc/models/list-team-member-wages-response.md)

## Example Usage

```php
$laborController = $client->getLaborController();

try {
    $result = $laborController->listTeamMemberWages();
    echo 'ListTeamMemberWagesResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Get Team Member Wage

Returns a single `TeamMemberWage` specified by `id `.

```php
function getTeamMemberWage(string $id): GetTeamMemberWageResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `TeamMemberWage` being retrieved. |

## Requires scope

### oauth2

`EMPLOYEES_READ`

## Response Type

**200**: Success

[`GetTeamMemberWageResponse`](../../doc/models/get-team-member-wage-response.md)

## Example Usage

```php
$id = 'id0';

$laborController = $client->getLaborController();

try {
    $result = $laborController->getTeamMemberWage($id);
    echo 'GetTeamMemberWageResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# List Workweek Configs

Returns a list of `WorkweekConfig` instances for a business.

```php
function listWorkweekConfigs(?int $limit = null, ?string $cursor = null): ListWorkweekConfigsResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `limit` | `?int` | Query, Optional | The maximum number of `WorkweekConfigs` results to return per page. |
| `cursor` | `?string` | Query, Optional | A pointer to the next page of `WorkweekConfig` results to fetch. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_READ`

## Response Type

**200**: Success

[`ListWorkweekConfigsResponse`](../../doc/models/list-workweek-configs-response.md)

## Example Usage

```php
$laborController = $client->getLaborController();

try {
    $result = $laborController->listWorkweekConfigs();
    echo 'ListWorkweekConfigsResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```


# Update Workweek Config

Updates a `WorkweekConfig`.

```php
function updateWorkweekConfig(string $id, UpdateWorkweekConfigRequest $body): UpdateWorkweekConfigResponse
```

## Authentication

This endpoint requires [oauth2](../../doc/auth/oauth-2-authorization-code-grant.md)

## Parameters

| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The UUID for the `WorkweekConfig` object being updated. |
| `body` | [`UpdateWorkweekConfigRequest`](../../doc/models/update-workweek-config-request.md) | Body, Required | An object containing the fields to POST for the request.<br><br>See the corresponding object definition for field details. |

## Requires scope

### oauth2

`TIMECARDS_SETTINGS_READ`, `TIMECARDS_SETTINGS_WRITE`

## Response Type

**200**: Success

[`UpdateWorkweekConfigResponse`](../../doc/models/update-workweek-config-response.md)

## Example Usage

```php
$id = 'id0';

$body = UpdateWorkweekConfigRequestBuilder::init(
    WorkweekConfigBuilder::init(
        'start_of_day_local_time6',
        'start_of_week2'
    )->build()
)->build();

$laborController = $client->getLaborController();

try {
    $result = $laborController->updateWorkweekConfig(
        $id,
        $body
    );
    echo 'UpdateWorkweekConfigResponse:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
}
```

