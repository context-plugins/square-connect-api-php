...
| `total` | `?int` | Optional | The exact amount in the `total_money` for a payment.<br><br>**Constraints**: `>= 0` | getTotal(): ?int | setTotal(?int total): void |

## Example

```php
use SquareConnectAPILib\Models\Builders\ListPaymentsRequestBuilder;

$listPaymentsRequest = ListPaymentsRequestBuilder::init()
