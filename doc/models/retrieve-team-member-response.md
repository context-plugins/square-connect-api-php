...
```php
use SquareConnectAPILib\Models\Builders\RetrieveTeamMemberResponseBuilder;
...
$retrieveTeamMemberResponse = RetrieveTeamMemberResponseBuilder::init()
    ->errors(
        [
            ErrorBuilder::init(
                'category8',
    ...
    ->teamMember(
        TeamMemberBuilder::init()
            ...
            ->status('ACTIVE')
            ->updatedAt('2020-06-11T22:55:45.867Z')
            ->build()
    )
    ->build();
```
