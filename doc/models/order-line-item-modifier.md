...
| `uid` | `?string` | Optional | A unique ID that identifies the modifier only within this order.<br><br>**Constraints**: *Maximum Length*: `60` | getUid(): ?string | setUid(?string uid): void |
...
$orderLineItemModifier = OrderLineItemModifierBuilder::init()
    ->basePriceMoney(
        MoneyBuilder::init()
            ->amount(114)
            ->currency('currency4')
